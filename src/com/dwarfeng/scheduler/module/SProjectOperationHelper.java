 ...
 * @since 1.8
 */
public final class SProjectOperationHelper {
	...
	public static void requestDelete(Deleteable deleteable){
		...
		if(sel == JOptionPane.YES_OPTION){
			...
			if(deleteable instanceof Editable){
				SProjectOperationHelper.forceDisposeEditor((Editable)deleteable);
			}
	...
	public static Project createNewProject(File file){
		...
		try{
			CT.trace("正在创建新文件...");
			return SProjectIoHelper.createDefaultProject(file);
		}catch(Exception e){
			e.printStackTrace();
			return null;
		}
	...
	public static Project loadProject(File file){
		...
		try {
			project = SProjectIoHelper.loadProject(file, SProjectIoHelper.Operate.FOREGROUND);
			CT.trace("文件读取成功");
			return project;
		}catch(Exception e){
	...
	public static void saveProject(Project project){
		...
		CT.trace("正在将工程压缩存储");
		try {
			SProjectIoHelper.saveProject(project,SProjectIoHelper.Operate.FOREGROUND);
			CT.trace("文件保存成功");
		}catch(Exception e){
			if(e instanceof IOException){
	...
	public static void closeProject(Project project){
		...
		CT.trace("正在释放工程文件");
		try{
			SProjectIoHelper.closeProject(project);
			CT.trace("工程文件释放完毕");
	...
	private SProjectOperationHelper(){
		//禁止实例化该类。
	}
}
