...
implements PopupInTree,Editable{
	...
	/**对象的XML附件*/
	private final PXmlAttachment attachment;
	...
	public static class Productor{
		...
		private List<PImprovisedPlant> improvisedPlains = new ArrayList<PImprovisedPlant>();
		 ...
		 * @return 构造器自身。
		 */
		public Productor improvisedPlains(List<PImprovisedPlant> val){
			if(val == null){
				this.improvisedPlains = new ArrayList<PImprovisedPlant>();
			}else{
		 ...
		 * @return 构造的即兴计划集合。
		 */
		public PImprovisedPlantCol product(){
			return new PImprovisedPlantCol(improvisedPlains);
		}
	 ...
	 */
	private PImprovisedPlantCol(List<PImprovisedPlant> improvisedPlains){
		super(true);
		//设置附件
		this.attachment = new PXmlAttachment(ATT_PATH);
		this.attachment.setContext(getRootProject());
		//将notebooks中的所有notebook添加进树中
		if(improvisedPlains != null){
			for(PImprovisedPlant improvisedPlain : improvisedPlains){
	 ...
	 * @return XML附件。
	 */
	public PXmlAttachment getAttachment(){
	 ...
	 */
	@Override
	public Editor<PImprovisedPlantCol> newEditor(){
	...
	private class ImprovisedPlantColEditor extends AbstractEditor<PImprovisedPlantCol> {
		...
		private ImprovisedPlantColEditor(){
			super(new PImprovisedPlantCol.Productor().product(),0);
		 ...
		 */
		public ImprovisedPlantColEditor(PImprovisedPlantCol editable) {
			super(editable);
		}
		...
		protected void editorInit() throws Exception {
			...
			plantButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					PImprovisedPlant improvisedPlant = new PImprovisedPlant
							.Productor(new PlainTextAttachment(SProjectIoHelper.genSchedulerURL(getEditable().getRootProject(), "implants" + File.separator,".txt")))
							.serialParam(sp)
							.product();
