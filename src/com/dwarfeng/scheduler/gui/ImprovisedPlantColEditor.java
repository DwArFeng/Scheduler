 ...
 * @since 1.8
 */
public class ImprovisedPlantColEditor extends AbstractEditor<ImprovisedPlantCol> {
	...
	private Im genIm(Element element) throws DocumentException{
		String imText = null;
		long pubTime = 0;
		Set<ImTag> tags = new HashSet<Im.ImTag>();
...
class Im{
	...
	public static class Productor{
		...
		private Set<ImTag> tags = new HashSet<Im.ImTag>();
		...
		public Productor tags(Set<ImTag> val){
			if(val == null){
				this.tags = new HashSet<Im.ImTag>();
			}else{
				this.tags = val;
			}
			return this;
	...
	public Set<ImTag> getImTags(){
		return new HashSet<Im.ImTag>(this.tags);
	}
