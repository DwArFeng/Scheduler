...
implements Editable, Deleteable, Moveable,PopupInTree,Searchable,SerialParamSetable {
	...
	public static class Productor{
		 ...
		 * @return 由构造器构造的即兴计划。
		 */
		public PImprovisedPlant product(){
			return new PImprovisedPlant(attachment, lineWrap, serialParam);
		}
	 ...
	 */
	private PImprovisedPlant(PlainTextAttachment attachment,boolean lineWrap,SerialParam serialParam) {
		super(false);
		//设置附件
		this.attachment = attachment;
	 ...
	 */
	@Override
	public Editor<PImprovisedPlant> newEditor(){
	...
	class ImprovisedPlantEditor extends AbstractEditor<PImprovisedPlant> {
		...
		private ImprovisedPlantEditor(){
			super(new PImprovisedPlant.Productor(null).product(),0);
			try {
				editorInit();
			} catch (Exception e) {
		 ...
		 */
		public ImprovisedPlantEditor(PImprovisedPlant editable) {
			super(editable);
		}
